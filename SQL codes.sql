...
--4a
select extract(year from occurred_at) as year, sum(total_amt_usd) as revenue,  
ROUND(((sum(total_amt_usd)-lag(sum(total_amt_usd)) over (order by extract(year from occurred_at)))/lag(sum(total_amt_usd)) over (order by extract(year from occurred_at))) * 100,2) as rev_growth_percent
from orders
where extract(year from occurred_at) > '2013'
and extract(year from occurred_at) < '2017'
group by extract(year from occurred_at);

--4b
select extract(year from occurred_at) as year, sum(total) as units_sold,  
ROUND(((sum(total)-lag(sum(total)) over (order by extract(year from occurred_at))::numeric)/ lag(sum(total)) over (order by extract(year from occurred_at)))*100,2) as units_sold_growth_percent
from orders
where extract(year from occurred_at) > '2013'
and extract(year from occurred_at) < '2017'
group by extract(year from occurred_at);
